...
//   // const fetchArea = async stateId => {
//   //   try {
//   //     const response = await axios.get(
//   //       `http://49.248.250.54:8081/api/GetArea?StateId=${stateId}&MasCityName=${cityName}`,
//   //       {
//   //         headers: {
//   //           Authorization:
//   //             'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3NDk5ODUzMTksImlzcyI6Imh0dHA6Ly9DaHVyY2guY29tIiwiYXVkIjoiaHR0cDovL0NodXJjaC5jb20ifQ.iJMILNigyJTfdp5LxKyMdcw8oHbUw3DqyMt5UJx0EjA',
...
//   //     );
//   //     if (response.data.status === 200) {
//   //       const mappedArea = response.data.data.map(area => ({
//   //         label: city.name,
//   //         value: state.id,
//   //       }));
//   //       setArea(mappedArea);
//   //       console.log(mappedArea); // Log the mapped list of cities to the console
//   //     } else {
//   //       console.error('Failed to fetch cities');
//   //     }
//   //   } catch (error) {
...
//   const fetchArea = async (stateId, cityName) => {
//     try {
//       const response = await axios.get(
//         `http://49.248.250.54:8081/api/GetArea?StateId=${stateId}&MasCityName=${cityName}`,
//         {
...
//       // Check HTTP status code instead of response.data.status if relevant
//       if (response.status === 200 && response.data.data) {
//         const mappedArea = response.data.data.map(area => ({
//           label: area.name,  // Assuming the area object has a 'name' field
//           value: area.id,    // Assuming the area object has an 'id' field
//         }));
//         setAreas(mappedArea);
//         console.log(mappedArea); // Log the mapped list of areas to the console
//       } else {
...
//     if (selectedState) {
//       fetchArea(selectedState.value, city.label);
//     }
//   };

//   // const onAreaChange = area => {
//   //   setSelectedArea(area);
//   //   // Check if 'area' is valid and fetch the areas
//   //   if (selectedArea) {
//   //     fetchChurches(selectedArea.value,area.name); // Use 'area' instead of 'selectedCity'
//   //   }
//   // };
...
//   //   setSelectedArea(area);
//   //   if (selectedArea) {
//   //     fetchChurches(selectedCity.value, area.label);
//   //   }
//   // };
...
//       fetchChurches(selectedState.value, selectedCity.label);
//     } else {
//       console.error('State or City is not selected.');
//     }
//   };
...
//         formData.append('stateFid', stateFid);
...
const SignUp: React.FC = () => {
  ...
  const [dateb, setDateb] = useState(new Date());
  const [show, setShow] = useState(false);
  const [stateFid, setStateFid] = useState([]);
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      try {
        ...
        formData.append('stateFid', selectedState?.value || '');
