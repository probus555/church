...
const EditProfileScreen = () => {
    ...
    const handleDateChange = (event: any, selectedDate?: Date) => {
        ...
        if (selectedDate) {
            if (role === 'Individual') {
                setIND_DOB(selectedDate);  // Update Individual DOB
            } else {
                setLead_DOB(selectedDate); // Update Leader DOB
            }
        }
    };
                ...
                <View style={{ position: 'relative' }}>
                    <Image
                        source={
                            file && file.path
                                ? { uri: file.path }
                                : file ? { uri: image } : images.appImages.signup
                        }
                        // source={
                        //     file && file.path
                        //         ? { uri: file.path }  // Use the image from the picker
                        //         : image
                        //         ? { uri: image }      // Fallback to the URL from employeeDetails
                        //         : images.appImages.signup  // Default image
                        // }
                        style={{
                            width: 125,
                            height: 125,
...
<View style={{ marginTop: 10, gap: 5, backgroundColor: theme.colors.background }}>
                <TouchableOpacity onPress={showDatepicker}>
                    <TextInput
                        style={{ backgroundColor: '#f1f1f1', color: theme.colors.onBackground,  }}
                        mode="flat"
                        editable={false}
                        placeholder="Date of Birth"
                        value={role === 'Individual' ? (IND_DOB ? IND_DOB.toISOString().split('T')[0] : '') : (lead_DOB ? lead_DOB.toISOString().split('T')[0] : '')}
                    />
                </TouchableOpacity>
            </View>
            ...
            {/* <Button
               ...
               mode="contained"
               icon="content-save"
                 style={{ marginTop: 20, backgroundColor: 'black',  padding: 5, }}
                onPress={handleSaveProfile}
            >
               SAVE
             </Button>
...
//                             file && file.path
//                                 ? { uri: file.path }
//                                 : image ? { uri: image } : images.appImages.signup
//                         }
//                         style={{
//                             width: 125,
//                             height: 125,
...
//                     <Text style={styles.datePickerText}>{dob ? dob.toDateString() : 'Select Date of Birth'}</Text>
//                 </TouchableOpacity>
//                 {showDatePicker && (
//                     <DateTimePicker
//                         value={dob || new Date()}
